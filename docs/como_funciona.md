...
0. **Recordatorio de contratación pendiente** — si el cliente ya *aceptó* esta oferta en el historial y nunca quedó como Movistar Total (ver 5.9 bis), es el lead más caliente que existe: siempre es rank 1, por delante de cualquier otro criterio.
1. **El valor esperado ajustado, agrupado a 2 decimales** — donde el modelo ve una diferencia real, el modelo manda. El "ajustado" es el VE descontado por rechazos previos de esa misma oferta a ese mismo cliente (ver 5.9 bis): un tier que el cliente ya rechazó por caro baja de grupo y le deja el paso al intermedio, que es exactamente lo que haría un buen asesor.
2. **Dentro del empate técnico, primero pierde el downgrade** — ni de precio (un plan móvil más barato que el actual puede "ganar" por desempate de ahorro, pero baja el ARPU, uno de los indicadores que la ficha pide mejorar) ni de **datos**: una oferta —incluida Movistar Total— que le da menos GB de los que el cliente realmente consume tampoco es una jugada proactiva. Esto último es nuevo: antes solo se detectaba downgrade de precio en planes móviles sueltos, así que un tier de MT con menos datos que el plan actual podía salir #1 igual, porque en precio "ganaba". Ahora se compara contra el consumo real con un margen de holgura (`consumo_gb × 1.2`), no contra el plan actual — perder el "ilimitado" nominal no es downgrade si el cliente apenas lo usa, y un tier con tope sigue compitiendo cuando de verdad le alcanza. Importante: el downgrade pierde *dentro de su empate*, no contra toda la lista — un tier de MT corto de GB queda detrás del tier que sí cubre el consumo, pero no sepultado bajo accesorios de conversión marginal.
3. **`avanza_a_mt`** — ¿la oferta es Movistar Total, o es el "producto puente" que cierra el gap de este cliente hacia MT? Es la razón de ser del proyecto: blindar la planta. Pero ya no puede empujar un downgrade por encima del tier que sí le alcanza al cliente.
4. Al final del empate: valor esperado ajustado exacto, luego el ahorro en soles, luego precio.
...
### 5.9 bis — El historial manda: recordatorios y rechazos

Dos señales del historial que el ranking ignoraba por completo hasta ahora, y que cambian tanto el orden como el approach del asesor:

**Contratación aceptada y nunca completada.** El Hallazgo 5 (ver [hallazgos_datos.md](hallazgos_datos.md)) documenta una trampa del dataset: de los 9,639 clientes que **aceptaron** una oferta de Movistar Total en el historial, los 9,639 siguen con `es_movistar_total = false` en el snapshot — la aceptación quedó registrada pero la contratación nunca se completó. Volver a "vender" esa misma oferta como si fuera nueva es leer mal la situación: el cliente ya dijo que sí una vez. El pipeline detecta estos casos (`pipeline/src/scoring.py::_recordatorios`) y, cuando la oferta aceptada sigue siendo elegible hoy, la marca con `accion = "recordatorio"` en vez de `"oferta"` — la recomendación es la MISMA, pero fuerza rank 1 y cambia el approach del copiloto de venta a cierre (retomar, confirmar interés, preguntar qué impidió completar la contratación, cerrar). Cuando la oferta aceptada ya no es elegible, no se fabrica una fila en el ranking: el caso queda visible en `journey.resumen.mt_aceptado_pendiente`.

**Rechazos previos de esa misma oferta.** Antes solo se usaba un rechazo por `mal_momento` para sugerir cuándo recontactar; el resto del historial de rechazos no influía en nada. Ahora cada rechazo de una oferta específica por un cliente específico descuenta su prioridad: `valor_esperado_ajustado = valor_esperado × 0.75^carga`, donde `carga` es la suma de los rechazos ponderados por antigüedad (media vida de 6 meses — un rechazo de hace un mes pesa ≈0.89, uno de hace seis pesa 0.5). No se excluye la oferta —puede seguir siendo la mejor opción disponible— pero baja en el orden, y el copiloto recibe el conteo y la fecha para pedir el motivo vía `get_journey` y ajustar el argumento en vez de repetir el mismo speech.

Ninguna de las dos señales entra al modelo (`pipeline/src/dataset.py::FEATURES_A/B` no las incluye): son política post-scoring, igual que el resto de esta sección, precisamente para no reentrenar nada ni arriesgar que el modelo vea en producción algo distinto a lo que aprendió.
...
- **El speech se escribe hablado, no escrito.** Un LLM sin instrucción produce un molde muy reconocible: abre validando la emoción ("entiendo perfectamente, señor(a)"), pivota con "justamente por eso" y cierra con una antítesis de eslogan ("no es pagar más, es ordenar lo que ya tiene"). Los números son correctos, pero un asesor no lee en voz alta algo que suena a folleto — y un speech que no se usa no sirve de nada. El prompt nombra esos tics uno por uno y prescribe la forma contraria: entrar por el dato, tres o cuatro frases. El `señor(a)` se elimina porque los datos no traen nombre ni sexo del cliente: el asesor ya sabe con quién habla.
- **Cada momento de la llamada tiene su speech.** La primera versión prescribía una única forma con cierre en pregunta, y el resultado en uso real fue un copiloto que soltaba el mismo speech —palabra por palabra, con el mismo "¿Se lo dejo activado hoy?"— para la apertura, para una objeción y para el cierre. El prompt ahora distingue los tres momentos (apertura: presentarse, motivo y gancho, sin cerrar la venta en la primera frase; objeción: responder primero a lo que el cliente dijo con la palanca del rebate; cierre: ahí sí la pregunta directa, y variada), prohíbe repetir un speech verbatim dentro de la conversación, y exige que después de la cifra venga el beneficio concreto del cliente — un speech vende, no lee la aritmética.
- **Un recordatorio no se re-vende.** Si la recomendación trae `accion: "recordatorio"`, el cliente ya aceptó esa oferta y la contratación quedó pendiente (ver 5.9 bis): el prompt prohíbe re-argumentar el precio o el ahorro como si fuera nueva y exige retomar, confirmar y cerrar.
- **Un downgrade de datos y un rechazo previo se advierten, no se esconden.** Si `es_downgrade_datos` es verdadero, el copiloto lo dice antes de ofrecerla; si `n_rechazos_previos` es mayor a cero, consulta el motivo con `get_journey` y ajusta el argumento a eso, en vez de repetir el speech que ya falló.
